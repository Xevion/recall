...
export const showCommand = new Command("show")
  ...
  .action(async (sessionId, opts) => {
    ...
    try {
      const [session] = await all(db, "SELECT * FROM session WHERE id = ? OR id LIKE ?", sessionId, `${sessionId}%`);
      if (!session) {
        console.error(`Session not found: ${sessionId}`);
        process.exit(1);
      }

      const analysis = await all(db, "SELECT * FROM analysis WHERE session_id = ?", (session as Record<string, unknown>).id);
      const subagents = await all(db, "SELECT id, title, message_count, turn_count FROM session WHERE parent_id = ?", (session as Record<string, unknown>).id);
      ...
      if (opts.json) {
        console.log(JSON.stringify({ session, analysis: analysis[0], subagents, toolStats }, null, 2));
      } else {
        ...
        if (subagents.length > 0) {
          ...
          for (const sa of subagents as Array<Record<string, unknown>>) {
            console.log(`  ${sa.id} — ${sa.title ?? "untitled"} (${sa.message_count} msgs)`);
          }
        }
        ...
        if ((toolStats as unknown[]).length > 0) {
          ...
          for (const t of toolStats as Array<Record<string, unknown>>) {
            const errStr = (t.errors as number) > 0 ? ` (${t.errors} errors)` : "";
