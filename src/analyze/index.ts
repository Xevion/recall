...
import { triageSession, type TriageInput } from "./triage";
import { buildAnalysisPrompt } from "./prompt";
import { analysisSchema, type AnalysisOutput } from "./schema";
import { isResearchByPrompt, extractResearchArtifact } from "./research";
...
export async function analyze(db: duckdb.Database, opts: AnalyzeOptions): Promise<AnalyzeResult> {
  const config = await loadConfig();
  const result: AnalyzeResult = { analyzed: 0, skipped: 0, errors: 0, refused: 0 };
  ...
  if (opts.force) {
    sessions = await all(db, "SELECT id, message_count, turn_count, duration_s, parent_id FROM session WHERE id = ?", opts.force);
    // Reset analysis status
    await run(db, "UPDATE analysis SET status = 'pending', retry_count = 0 WHERE session_id = ?", opts.force);
  } else {
    sessions = await all(
      db,
  ...
  for (const session of sessions) {
    const hasToolCalls = (await all(db, "SELECT 1 FROM tool_call WHERE session_id = ? LIMIT 1", session.id)).length > 0;
    const hasSubagents = (await all(db, "SELECT 1 FROM session WHERE parent_id = ? LIMIT 1", session.id)).length > 0;
    const hasErrors = (await all(db, "SELECT 1 FROM tool_call WHERE session_id = ? AND is_error = TRUE LIMIT 1", session.id)).length > 0;
    ...
    if (decision === "skip") {
      await run(db, "UPDATE analysis SET status = 'skipped' WHERE session_id = ?", session.id);
      result.skipped++;
    } else {
  ...
  for (const chunk of chunks) {
    if (consecutiveFailures >= config.analyze.max_consecutive_failures) {
      console.error(`Circuit breaker: ${consecutiveFailures} consecutive failures, aborting.`);
      break;
    }

    const results = await Promise.all(
      chunk.map((session) => analyzeSession(db, session.id, session.parent_id, config)),
    );
    ...
    if (config.analyze.delay_ms > 0 && chunks.indexOf(chunk) < chunks.length - 1) {
...
async function analyzeSession(
  ...
  try {
    await run(db, "UPDATE analysis SET status = 'processing' WHERE session_id = ?", sessionId);
    ...
    console.log(`[analyze] Would analyze session ${sessionId} (${prompt.length} chars)`);
    return "complete";
  } catch (err) {
