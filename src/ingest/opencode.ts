...
import type { NormalizedSession, NormalizedMessage, NormalizedToolCall, NormalizedSubagent } from "./types";
...
export async function ingestOpenCode(
  ...
  try {
    const sessions = sqlite.query<
      ...
      []
    >("SELECT id, project_id, parent_id, title FROM session").all();
...
function parseOpenCodeSession(
  sqlite: SQLiteDB,
  ocSession: { id: string; project_id: string; parent_id: string | null; title: string | null },
  sessionId: string,
): NormalizedSession | null {
  // Get messages for this session
  const ocMessages = sqlite.query<
    { id: string; session_id: string; role: string; model_id: string | null },
    [string]
  >("SELECT id, session_id, role, model_id FROM message WHERE session_id = ? ORDER BY rowid", ocSession.id).all();
  ...
  for (let seq = 0; seq < ocMessages.length; seq++) {
    ...
    const parts = sqlite.query<
      ...
      [string]
    >("SELECT id, type, data FROM part WHERE message_id = ? ORDER BY rowid", ocMsg.id).all();
      ...
      try {
        ...
        if (part.type === "text") {
          ...
          toolCalls.push({
            ...
            inputSummary: data.args ? JSON.stringify(data.args).slice(0, 100) : null,
  ...
  return {
    ...
    parentId,
    projectPath: null,
    projectName: ocSession.project_id === "global" ? null : ocSession.project_id,
    gitBranch: null,
...
async function persistOpenCodeSession(db: duckdb.Database, session: NormalizedSession): Promise<void> {
