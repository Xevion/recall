 ...
 */
export async function buildAnalysisPrompt(db: duckdb.Database, sessionId: string): Promise<string> {
  ...
  if (subagent?.prompt) lines.push(`Dispatch prompt: ${subagent.prompt.slice(0, 500)}`);
  lines.push("");
